<template>
  <div class="zhuye">
    <div class="img2">
      <img
        ...
        alt="icon"
      />
      <div id="jian">have nothings</div>
      <div>
        <div></div>
        <span class="jian">tianmen</span>
      </div>
      <div class=".body">
        <div class="foot">
          <a href="https://space.bilibili.com/660045843">
            <div class="svg">
              <svg
                width="1.5em"
                height="1.5em"
          ...
          <a href="https://github.com/tianmen114">
            <div class="svg">
              <svg
                width="1.5em"
                height="1.5em"
  ...
  <div class="sidebar">
    <h2>模型选择</h2>
    <!-- 模型选择下拉框 -->
    <select v-model="selectedModel" @change="saveSelectedModel">
  ...
  <textarea
    class="shuru"
    v-model="userMessage"
    placeholder="输入你的问题主人喵~"
    rows="4"
    @keydown.enter.prevent="sendMessage"
  ></textarea>
  <!-- 发送按钮 -->
  <button class="an" :disabled="loading" @click="sendMessage">
    {{ loading ? "生成中喵~" : "发送" }}
    <div class="id">
      <!-- 回复显示 -->
...
.zhuye:hover {
  box-shadow: 0px 0px 4px 4px #f4a3b3;
  transition: all 0.3s ease;
}
...
.svgback {
  ...
  border-radius: 50%;
  width: 50px;
  background-color: rgba(236, 169, 181, 0.266);
  z-index: -1;
  transition: all 0.3s ease;
}

.svg:hover .svgback {
  background-color: rgba(236, 169, 181, 0.656);
  transition: all 0.3s ease;
}
