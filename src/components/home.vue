    ...
    <div v-else class="article-grid">
      <div v-for="article in paginatedArticles" :key="article.id" class="article-card" @click="viewArticle(article.id)">
        ...
        <div class="article-info">
          <h3>
            {{ article.title }}
            <span v-if="article.top !== undefined" class="top-tag"><svg width="1em" height="1em" class="inline text-[var(--primary)] text-2xl mr-2 -translate-y-0.5" data-astro-cid-iyiqi2so="true" data-icon="mdi:pin">   <symbol id="ai:mdi:pin" viewBox="0 0 24 24"><path fill="currentColor" d="M16 12V4h1V2H7v2h1v8l-2 2v2h5.2v6h1.6v-6H18v-2z"></path></symbol><use href="#ai:mdi:pin"></use>  </svg></span>
            <div class="xiaobiao"> &nbsp;></div>
          </h3>
          <div class="meta">
            <span class="fontcolor"><svg width="1.1em" class="svg" viewBox="-43 -17 536 536.00104"
                ...
                <path
                  ...
                  fill="#000000" style="fill: rgb(207, 108, 127);"></path>
              </svg><span class="date"> {{ formatDate(article.date) }}</span>
            </span>
            <div class="tags">
        ...
        <p class="description">{{ article.description }}</p>              <div class="zishu">{{ article.title.replace(/\s+/g, '').length +
                article.description.replace(/\s+/g, '').length + article.content.replace(/\s+/g, '').length +" 字"}}</div>
      </div>
    </div>
